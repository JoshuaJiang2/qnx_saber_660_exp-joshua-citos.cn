...
/*
这个函数会读取image_header.S中的dcd table，然后写寄存器
table example: DCD_ENTRY_QUAD(1, MX6X_IOMUXC_BASE  + 0x5a8, 0x00000028)
---joshua@citos.cn
*/
...
void init_pinmux_dual_lite_solo()
{
    ...
    /* UART4 TX - pin KEY_COL0 */
	set_mux_cfg(SWMUX_SDL_KEY_COL0, MUX_CTL_MUX_MODE_ALT4);
	set_pad_cfg(SWPAD_SDL_KEY_COL0, MX6Q_PAD_SETTINGS_UART);

	/* UART4 RX - pin KEY_ROW0 */
	set_mux_cfg(SWMUX_SDL_KEY_ROW0, MUX_CTL_MUX_MODE_ALT4);
	set_pad_cfg(SWPAD_SDL_KEY_ROW0, MX6Q_PAD_SETTINGS_UART);
	set_pin_as_input(SWINPUT_SDL_UART4_IPP_UART_RXD_MUX, 0x03);


	/* UART2*/
	set_mux_cfg(SWMUX_SDL_GPIO_7, MUX_CTL_MUX_MODE_ALT4);
	set_pad_cfg(SWPAD_SDL_GPIO_7, MX6Q_PAD_SETTINGS_UART);
	set_pin_as_input(SWINPUT_SDL_UART2_IPP_UART_RXD_MUX, 0x2);

	set_mux_cfg(SWMUX_SDL_GPIO_8, MUX_CTL_MUX_MODE_ALT4);
	set_pad_cfg(SWPAD_SDL_GPIO_8, MX6Q_PAD_SETTINGS_UART);
	set_pin_as_input(SWINPUT_SDL_UART2_IPP_UART_RXD_MUX, 0x3);

	set_mux_cfg(SWMUX_SDL_SD4_DAT6, MUX_CTL_MUX_MODE_ALT2);
	set_pad_cfg(SWPAD_SDL_SD4_DAT6, MX6Q_PAD_SETTINGS_UART);
	set_pin_as_input(SWINPUT_SDL_UART2_IPP_UART_RTS_B, 0x5);

	set_mux_cfg(SWMUX_SDL_SD4_DAT5, MUX_CTL_MUX_MODE_ALT2);
	set_pad_cfg(SWPAD_SDL_SD4_DAT5, MX6Q_PAD_SETTINGS_UART);
	set_pin_as_input(SWINPUT_SDL_UART2_IPP_UART_RTS_B, 0x4);
